      ...
      <div className="relative mx-auto max-w-5xl space-y-6">
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div>
              ...
              <h1 className="mt-2 text-3xl font-bold text-[#24312A]">
                Agendar cita con nutrición
              </h1>
              <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600">
                Programa una cita con la misma nutricionista para un cliente existente o uno recién creado.
              </p>
            </div>
        ...
        <section className="rounded-3xl border border-[#D6E8DA] bg-white p-6 shadow-sm">
          ...
          <p className="mt-1 text-sm text-slate-500">
            Esta pantalla queda preparada para luego conectar disponibilidad real de agenda.
          </p>

          <form className="mt-5 space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              <Field label="Buscar cliente existente" />
              <Field label="Nutricionista" defaultValue="Nutricionista actual" />
              <Field label="Fecha" type="date" />
              <Field label="Hora" type="time" />
              <Field label="Tipo de cita" defaultValue="Valoración nutricional" />
              <Field label="Estado inicial" defaultValue="Agendada" />
            </div>
            ...
            <div className="grid gap-3 md:grid-cols-3">
              <button
                type="button"
                className="rounded-2xl bg-[#0DA56F] px-4 py-4 text-base font-semibold text-white transition hover:bg-[#0B8E5F]"
              >
                Guardar cita
              </button>
...
function Field({
  ...
  return (
    <div>
      ...
      <input className={inputClass} type={type} defaultValue={defaultValue} />
    </div>
  );
}
